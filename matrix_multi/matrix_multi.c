...
void* worker_multiply(void* args) {
	...
	// each thread gets a seperate column (or set of columns) of a and a seperate set of rows in b
	//of the same size
	const size_t start = wargs->id * CHUNK;

	// if it is the last chunk it maye no be a multiple so we extend its work
